...
export function useDrawingMode(
    ...
    setTempRoutePoints((prev) => {
      ...
      if (map && !map.getSource(pointId)) {
        map.addSource(pointId, {
          type: "geojson",
          data: { type: "FeatureCollection", features: [{ type: "Feature", geometry: { type: "Point", coordinates: newPoint }, properties: null }] },
        });
        map.addLayer({
          id: pointId,
          type: "circle",
  ...
  const finishDrawing = useCallback(() => {
    ...
    draw.add({ type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: tempRoutePoints } });
  ...
  useEffect(() => {
      ...
      setTempRoutePoints((prev) => {
        ...
        if (draw) {
          ...
          if (newPoints.length >= 2) {
            draw.add({ type: "Feature", properties: { temp: true }, geometry: { type: "LineString", coordinates: newPoints } });
          }
        }
        ...
        if (!map.getSource(pointId)) {
          ...
          map.addLayer({ id: pointId, type: "circle", source: pointId, paint: { "circle-radius": 5, "circle-color": "#facc15", "circle-stroke-color": "#fff", "circle-stroke-width": 1.5 } });
        }
    ...
    const updatePreview = (lng: number, lat: number) => {
      ...
      if (draw) {
        draw.getAll().features.filter((f) => f.properties?.temp).forEach((f) => draw.delete(f.id));
        draw.add({ type: "Feature", properties: { temp: true }, geometry: { type: "LineString", coordinates: previewCoords } });
      }
    };
  ...
  }, [isDrawingMode, tempRoutePoints, finishDrawing, cancelDrawing]);
